...
def lineplotMD(x_data, y1_data, y2_data, y3_data, y4_data, x_label="", y_label="", title=""):
    _, ax = plt.subplots()
    ax.plot(x_data, y1_data, 'b-.', lw=3, label=('Формула для пакетной системы'))
    ax.plot(x_data, y2_data, 'g-.', lw=3, label=('Экспоненциальное распределение с 1'))
    ax.plot(x_data, y3_data, 'r-.', lw=3, label=('Фиксированное значение 1'))
    ax.plot(x_data, y4_data, 'm-.', lw=3, label=('Равномерное распределение с 1'))
    ax.set_title(title)
